...
Phase 6QB 由三個 `luna_worker` 完成 host-only 搜尋，再以一份 31-command
read-only baseline 交叉驗收。新增 18-row residual matrix：2 個
`IAmazonPackageManager` caller rows、9 個 Vending downstream rows、7 個
PS7331 Framework/OTA residual writer rows。沒有發現新的低權限 caller 到
User-0 PackageManager、HOME、Fire Launcher、system/root 或 partition sink 的
閉合鏈。

實機仍為 PS7331.4463N、SELinux Enforcing；HOME 為 Fire Launcher priority 50，
shell 對 Amazon private service 的 `service_manager find` 仍被拒絕。tx6/tx7
production caller、Vending injected writer、OOBE numeric-user mapping、OTA→recovery
caller provenance 仍標為 UNKNOWN，不執行 private Binder、protected broadcast、
OTA/recovery 或 package mutation。
...
Phase 6QF 將研究問題擴展到所有已保存的高影響權限面，而不只限於 Launcher：
Amazon IPC caller provenance、PS7331 exact-image SELinux/client mapping，以及
既有 package/KFT/DPM/Accessibility/OOBE/OTA/driver runtime evidence。三份 ledger
正規化為 26 rows（12 IPC、7 policy/client、7 existing runtime）。

有界結論仍是：沒有閉合出 ordinary app/shell → accepted gate → system/root →
PackageManager/HOME/package-state/credential/SELinux/OTA/partition sink 的鏈。
`UNKNOWN` 保留為證據缺口，不視為漏洞。Phase 6QE 的 exact-device metadata-only
snapshot 被重用；本輪沒有新增 device mutation、private Binder、driver ioctl、OTA、
Root、reboot 或 partition operation。
...
PS7331 OTA/source scope 15、kernel/native residual 11；另建立指定序號的唯讀
PS7331 snapshot。H2ClientService 的 user creation/removal、per-profile Settings
relay 與 sort-order sink 是 production static chain，但 bind permission、外部
caller 與低權限可達性仍 `UNKNOWN`，沒有 formal HOME/PMS sink。OTA/recovery writer
是 privileged capability；ION 僅到 library-level positive，其餘 driver surface
缺 exact shipped caller 或 final policy/effect join。

目前設備仍為 `KFTRWI`/`trona`、PS7331.4463N、verified boot green，User 0 HOME
仍為 `com.amazon.firelauncher/.Launcher` priority 50。child HOME 與 foreground
redirect 仍分別是 user-scoped 或 temporary，沒有新的 User 0 durable replacement。

本輪未執行 Root/exploit、未知 Binder transaction、driver open/ioctl、OTA/recovery、
reboot、package/settings mutation 或分割區寫入；raw snapshot 只保留本機，公開內容
為 redacted summary、表格、worker 原始 ledger、雜湊 manifest 與可重現腳本。
