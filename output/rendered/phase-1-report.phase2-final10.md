...
Latest read-only verification: `device/baseline/BASELINE-20260803-07/` and `adb/baseline/BASELINE-20260803-07/`; both SHA-256 manifests passed. The device still resolves HOME to Fire Launcher and is restored to Fire Launcher foreground after the controlled tests.
...
The same gate applies when the shell targets only the HOME component. `pm disable-user` and `cmd package disable-user` for `com.amazon.firelauncher/com.amazon.firelauncher.Launcher` both return the same protected-package exception; the component's `disabledComponents` state, HOME resolver and foreground focus remain unchanged in `COMPONENT-T01` and `COMPONENT-T02`.

`Confirmed environment fact`: this device also has `com.amazon.parentalcontrols` as the User 0 Profile Owner. The read-only `POLICY-T01` dump shows no Device Owner, `Device managed: false`, and no effective user restrictions. The active Profile Owner is therefore a relevant environmental factor to record, but it is not the observed cause of the Fire disable exception: the tested exception stack terminates in `PackageManagerService.setEnabledSetting()` before any DevicePolicy path appears.
...
`Confirmed`: disabling only `com.amazon.firelauncher/.Launcher` is not an ADB workaround on this build. Both package-manager shell entry points reject the component at `setComponentEnabledSetting()`.
...
- `device/baseline/BASELINE-20260803-05/sha256sums.txt`
- `device/baseline/BASELINE-20260803-06/sha256sums.txt`
- `device/baseline/BASELINE-20260803-07/sha256sums.txt`
- `adb/baseline/BASELINE-20260803-04/sha256sums.txt`
- `adb/baseline/BASELINE-20260803-06/sha256sums.txt`
- `adb/baseline/BASELINE-20260803-07/sha256sums.txt`
...
| DevicePolicy / parental controls | User 0 has `com.amazon.parentalcontrols` as Profile Owner; its APK can set application restrictions on Fire Launcher for content/policy keys, but the inspected package-change and boot paths do not disable or re-enable Fire Launcher | Confirmed environment and code paths; not the tested `pm disable-user` cause |
| AmazonUserManagerService | Special KFT child-user upgrade path enables FreeTime Launcher and sets Fire Launcher/Launcher3 enabled state to `2` for that user | Confirmed code path; not the current normal-user path and not a watchdog restoring Fire |
...
### DevicePolicy and Amazon private package-management paths

`POLICY-T01` is a read-only environment capture. It reports `com.amazon.parentalcontrols/.receivers.ParentalAdminReceiver` as the Profile Owner for User 0, while `user.txt` reports `Device owner id:-10000`, `Device managed: false`, and effective restrictions `none` (`adb/probes/POLICY-T01/device_policy.txt:1-12,45-88`; `user.txt:9-28`).

The inspected parental-controls APK does contain Fire Launcher references, but they are application-level restrictions rather than the package-enabled-state operation used by `pm disable-user`: `ParentalControlsConstant.LAUNCHER_PACKAGE` is `com.amazon.firelauncher` (`decompiled/jadx/parentalcontrols/sources/com/amazon/parentalcontrols/constant/ParentalControlsConstant.java:67-70`), `PolicyAppRestriction.enforcePolicy()` calls `DevicePolicyManager.setApplicationRestrictions()` (`decompiled/jadx/parentalcontrols/sources/com/amazon/parentalcontrols/policy/policyImpl/PolicyAppRestriction.java:33-48`), and `ParentalAdminUtils.applyLauncherPolicyIfApplicable()` applies a content-policy key (`decompiled/jadx/parentalcontrols/sources/com/amazon/parentalcontrols/admin/ParentalAdminUtils.java:201-252`).

The package-change receiver filters changed packages through `ApplicationMetadataUtils.is3PApp()` before calling `setApplicationHidden()` (`decompiled/jadx/parentalcontrols/sources/com/amazon/parentalcontrols/receivers/PackageChangesReceiver.java:22-77`; the hidden-state implementation is `policy/policyImpl/PolicyAppHidden.java:19-33`), and the boot receiver only updates parental notification and curfew scheduling (`decompiled/jadx/parentalcontrols/sources/com/amazon/parentalcontrols/receivers/ParentalBootBroadcastReceiver.java:15-22`). The manifest marks that package-change receiver disabled by default (`decompiled/jadx/parentalcontrols/resources/AndroidManifest.xml:359-365`). These paths do not explain the synchronous Fire Launcher disable exception.

There is a separate special-user path in Amazon's `AmazonUserManagerService`: `enableKftLauncherComponent()` enables `com.amazon.tahoe/.launcher.FreeTimeLauncherActivity` and passes enabled state `2` to `setApplicationEnabledSetting()` for `com.amazon.firelauncher` and `com.android.launcher3` (`decompiled/baksmali/vdexExtractor/fosservices/disassembly.log:54297-54325`). The caller is `tryEnableKftLauncherComponent()`, used by `enableKftLauncher()` and the upgrade-time child-user loop (`:54371-54414,55099-55100`). This is an intentional FreeTime child-user switch, not evidence of a general watchdog that restores Fire Launcher for the current primary user.

Therefore the current disposition is: `Confirmed` DPM is present; `Confirmed` the parental-controls APK can constrain Fire Launcher content; `Confirmed` the tested `pm`/`cmd package` disable denial is the PackageManager protected-package path; `Disproved for this disable path` that the active Profile Owner is the immediate cause. DPM effects outside this request remain bounded unknowns.
...
Final read-only verification after all tests: `adb/baseline/BASELINE-20260803-05/home_resolve_cmd.txt:1-2` returns Fire priority 50; `device/baseline/BASELINE-20260803-05` and `adb/baseline/BASELINE-20260803-05` hash manifests pass. The same final snapshot reports `com.amazon.firelauncher/.Launcher` as `mResumedActivity` at `adb/baseline/BASELINE-20260803-05/dumpsys_activity_activities.txt:60,209` and as both `mCurrentFocus` and the `mFocusedApp` token at `adb/baseline/BASELINE-20260803-05/dumpsys_window_windows.txt:282-283`.
...
| `COMPONENT-T01` | `pm disable-user --user 0 com.amazon.firelauncher/com.amazon.firelauncher.Launcher` | Same protected-package exception; component state and HOME unchanged | Confirmed |
| `COMPONENT-T02` | `cmd package disable-user --user 0 com.amazon.firelauncher/com.amazon.firelauncher.Launcher` | Same protected-package exception; component state and HOME unchanged | Confirmed |
| `POLICY-T01` | Read-only `dumpsys device_policy`, `dumpsys user` and relevant package dumps | Parental Controls is Profile Owner; no Device Owner, device-managed state or effective user restrictions | Confirmed environment fact |
...
Settings picker route probes:

| Test ID | Route | Result | Status |
|---|---|---|---|
| SETTINGS-T01 | Shell launch of `SubSettings` with `DefaultHomePicker` fragment | Permission denied because `com.android.settings/.SubSettings` is not exported | Confirmed boundary; picker not executed |
| SETTINGS-T02 | Initial `AdvancedAppsActivity` attempt | Nested-class shell quoting expanded incorrectly; retained as a command-quoting artifact | Disqualified; not causal evidence |
| SETTINGS-T03 | Correctly quoted `Settings$AdvancedAppsActivity` with `DefaultHomePicker` fragment extra | Activity starts, but the metadata-selected Default apps dashboard remains visible and has no Home row | Confirmed for tested route |
...
The retained picker was also tested as an external entry-point hypothesis. `SETTINGS-T01` could not start `SubSettings` from shell because the manifest declaration has no `exported="true"`/intent-filter (`adb/settings-tests/SETTINGS-T01/open_fragment.txt`; `decompiled/jadx/settings/resources/AndroidManifest.xml:232-234`), even though the subclass accepts arbitrary fragments in its `isValidFragment()` override (`SubSettings.java:6-16`). `SETTINGS-T03` correctly quoted the nested `Settings$AdvancedAppsActivity` component and started it successfully, but the live activity remained the metadata-selected Default apps dashboard; `SettingsActivity.isValidFragment()` allows only `SettingsGateway.ENTRY_FRAGMENTS`, which includes `DefaultAppSettings` but not `DefaultHomePicker` (`decompiled/jadx/settings/sources/com/android/settings/SettingsActivity.java:403-410`; `core/gateway/SettingsGateway.java:131`). `SETTINGS-T02` is retained only as a command-quoting artifact. These tests make a standard external route to the retained picker `Probable unavailable`, but do not claim that no privileged/internal caller could reach it.
...
| Component-only disable | Both `pm` and `cmd package` reject `com.amazon.firelauncher/.Launcher` at `setComponentEnabledSetting()` | Disproved as an ADB workaround |
| DevicePolicy / parental-controls route | Active Profile Owner can set Fire Launcher application restrictions, but no tested DPM API changes the HOME resolver or bypasses PackageManager protection | Not a launcher workaround; DPM effect limited to separate policy surfaces in this evidence |
| Retained `DefaultHomePicker` | Picker code/resources remain, but tested external Settings routes are blocked or fall back to the Home-less Default apps dashboard | Probable unavailable through standard shell-visible Settings routes |
...
- What exact Fire Launcher application-restriction bundle is stored under the active parental-controls Profile Owner, and is parental control enabled for the current profile? The APK shows the capability, but the read-only shell dump does not expose per-package application-restriction contents.
...
tools/scripts/test_component_disable.sh \
  ...
  --user 0 \
  --command cmd \
  --output adb/component-tests/COMPONENT-REPRO-T01 \
  --approve-state-change
...
- Parental Controls: `0311dda6b8be16891e7052f1d0c4d94c4cc5ef4010fd6645b58ba72a192e92a5` (`firmware/manifests/ARTIFACT-20260803-08/sha256sums.txt`)
...
- Component protection probes: `adb/component-tests/COMPONENT-T01/`, `adb/component-tests/COMPONENT-T02/`
...
- Decompiled APKs: `decompiled/jadx/` (JADX completed for the readable APKs, including `parentalcontrols/`)
...
- Component protection finding: `findings/component-protection.md`
...
Generated evidence-index SHA-256: [23b20e1351060271d74e6396d0f460b4891df239318374a4eee13acbb6d1c14d]
