...
`Probable`: no stable no-root default-home workaround has been identified on this build. A physical-button capture and exact-match OTA comparison remain pending; an adjacent official PS7331 OTA and two AOSP Android 9 references are now available as explicitly version-mismatched/reference-only material.
...
Final read-only verification after all tests: `adb/baseline/BASELINE-20260803-03/home_resolve_cmd.txt:1-2` still returns Fire priority 50; `dumpsys_activity_activities.txt:60,183-190` and `dumpsys_window_windows.txt:284-285` show Fire foreground with `mKeyguardShowing=false`. Both final baseline hash manifests pass.
...
`tools/tool_versions.txt` records the original environment inventory; `tools/tool_versions.phase2.txt` records the follow-up inventory after adding the OTA/AOSP analysis tooling. This phase does not claim an exact-match OTA. The framework findings for the installed build use device-pulled VDEX/ODEX disassembly and JADX resources/source; PS7331 outputs are explicitly adjacent-version references.

---

Generated evidence-index SHA-256: [b40be829a8bc5feea2b1eb0620945e2b6aac8480be47b286ed55d38370854cc6]
