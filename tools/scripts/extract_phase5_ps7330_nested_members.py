#!/usr/bin/env python3
"""Extract selected PS7330 kernel members from Amazon's nested platform.tar.

Host-only provenance helper. It reads an official source archive, streams the
outer platform.tar member, extracts an allow-listed set of text members, and
...
def main() -> int:
    ...
    archive_hash = archive_sha256(args.archive)
    ...
    (args.output / "result.md").write_text(
        "# PS7330 nested source extraction\n\n"
